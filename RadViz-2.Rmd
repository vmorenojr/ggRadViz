...
In this study, we implement and compare two of the most recent proposed methods to mitigate the weaknesses of RadViz charts, namely Pagliosa and Telea's [-@pagliosa2019] and Cheng, Xu and Mueller's [-@cheng2017]. In particular, we look into the reordering techniques that try to improve the placement of dimensional anchors in the circunference of the chart to help better identify clusters of observations and outliers. We then compare the results with those obtained with the 'Radviz' package and with parallel coordinates, to highlight the strenghts and weakenesses of both visualization techniques. 
...
```{r iris_manual, fig.height = 7, fig.width = 7, fig.align='center'}
...
We removed the colors of the RadViz chart to check how one would interpret its information if the Species were not identified.
...
As before, two clusters are easily identified. In addition, there may be one or more outliers with values of Sepal.Width much higher than values for the other three variables. Unfortunately, the`Radviz` package does not provide any means to identify those observations or mark them in the chart.
...
There were no changes in the previously defined anchors. We repeated the process for the radviz-dependent method.
...
make_da <- function(dimensions){
  n <- length(dimensions)
    ...
    mutate(label = dimensions,
           nudge_x = ifelse(x>0, .3, ifelse(x==0, 0, -.3)),
...
Next, Pagliosa and Telea [-@pagliosa2019] use the normalized correlation matrix as a distance matrix and use it as the input to the average-linkage Agglomerative Hierarchical Clustering (AHC) algorithm. The dimensional anchors are placed around the RadViz circle in the order they appear in the resulting AHC dendogram.
...
## Cheng, Xu and Mueller's reordering method


Cheng, Xu and Mueller's [-@cheng2017]
